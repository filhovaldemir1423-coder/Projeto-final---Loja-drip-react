...
const RelatedProducts = ({currentProduct, allProduct}) => {
    
    if(!currentProduct || !allProduct || !Array.isArray(allProduct)) {
      return null;
    }
    
    const related = allProduct.filter(p => p.category === currentProduct.category && p.id !== currentProduct.id
    ).slice(0, 4);
