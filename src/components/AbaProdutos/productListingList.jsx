...
return(
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        ...
        {products.map((product) =>{
            ...
            return (
                <div key={product.id} className="bg-white border border-gray-200 rounded-lg shadow-md flex flex-col 
                justify-between transition-shadow hover:shadow-xl">
                    <Link to={`/produto/${product.id}`} className="block relative">
                    {hasDiscount && (
                        <div className="absolute top-3 left-3 bg-green-200 text-green-800 
                        text-xs font-bold px-3 py-1 rounded-md z-10">
                    ...
                    <div className="p-2 flex flex-col flex-grow">
                        ...
                        <h3 className="text-base font-semibold text-gray-800 mb-1 flex-grow">{product.name}</h3>
                    ...
                    <div className="px-2 pb-2">
                        ...
                        <button onClick={handleToggleCart} className={`w-full font-semibold py-1.5 text-sm rounded-lg transition mb-2 
                        ${ // ALTERAÇÃO: py-2 para py-1.5
                  isInCart
                ...
                <Link to={`/produto/${product.id}`} className="w-full block text-center bg-gray-200 text-gray-700 font-semibold 
                py-1.5 text-sm rounded-lg hover:bg-gray-300 transition" 
                > 
                     Ver mais
               </Link>
            </div>
